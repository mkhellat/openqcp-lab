...
def fwht_pauli_coefficients_sparse_v5(operator):
    """Only materializes work for ACTIVE x rows (those with >=1 nonzero
    gather entry), and only builds the phase factor for those rows too
    -- avoids ANY full (dim,dim) array entirely. Returns a dense
    (dim,dim) array for API compat with the existing dense function
    (Phase 3b step: keep the output contract identical for now; a
    later step could return the sparse form directly to
    fwht_pauli_terms without ever densifying)."""
    ...
    and_mat = q_sorted[:, None] & z_all[None, :]
    ...
    xz_and = unique_x[:, None] & z_all[None, :]  # only active rows!
    phase = 1j ** popcount_fast(xz_and).astype(np.int64)
    active_coeffs = grouped * np.conj(phase) / dim  # (n_active, dim)
...
if __name__ == "__main__":
    for n_osc in [16, 30, 50, 100]:
        ...
        sp = fwht_pauli_coefficients_sparse_v5(padded)
