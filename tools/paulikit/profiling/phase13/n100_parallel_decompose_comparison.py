"""Real N=100 wall-clock comparison: sequential fwht_pauli_terms_iter
(auto-tuned chunk_size, single process) vs. parallel_decompose (same
chunk_size, multi-process) - PLAN.md Phase 13a's first real
measurement, not just unit-test correctness. Confirms both identical
term counts (correctness) and measures the real speedup (not assumed
from core count - Amdahl's law/cache contention both apply, per the
scoping doc's own flag).
"""
import time
